...
const CLOUD_SAVE_ENDPOINT = "/api/vanishoot/save";
...
async function hydrateCloudSave() {
  try {
    ...
    meta.gold = Math.max(Number(meta.gold || 0), Number(save.gold || 0));
    meta.upgrades = {
      ...(save.upgrades || {}),
      ...(meta.upgrades || {}),
    };
    ...
    // Cloud save is optional; local play should never fail because sync failed.
  }
}
...
async function syncCloudSave() {
  try {
    ...
    // Keep local save authoritative if the network or auth session is unavailable.
  }
}
